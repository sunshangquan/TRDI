...
class SDInversionPipeline(StableDiffusionPipeline):
    ...
    def compute_captial_g_zt(
        ...
        alpha_t_prev = self.scheduler.alphas_cumprod[prev_timestep]
        pdf = -0.5 * torch.pow((cur_sample - 0.5 ** (alpha_t/alpha_t_prev) * prev_sample) / (0.5 ** (1 - alpha_t/alpha_t_prev)), 2)
        return pdf
    
    def guided_newton_rhapson_iter(
        self,
        z_t: torch.Tensor,
        timestep: torch.Tensor,
        prompt_embeds: torch.Tensor,
        ...
        for _ in range(num_iter_steps):
            ...
            f = self.compute_f_zt(z_t, latent, timestep, prompt_embeds, timestep_cond, added_cond_kwargs)
            capital_g = self.compute_captial_g_zt(z_t, latent, timestep)
            capital_f = (latent - f).abs() + param_lambda * capital_g

            l = capital_f.sum()
            score = capital_f.mean()
            if score < best_score:
                best_score = score
                best_latent = f.detach()
            l.backward()

            dim = latent.shape[0] * latent.shape[1] * latent.shape[2] * latent.shape[3]
            latent = latent - 1 / dim * capital_f / (latent.grad + param_eta)
            latent.grad = None
    ...
    def inverse(
        ...
        **kwargs,
        ...
        with EmojiProgressBar(total=num_inference_steps, desc="GNRI") as pbar:
            for i in range(num_inference_steps):
                t = timesteps[num_inference_steps-i-1]

                latents = self.guided_newton_rhapson_iter(
                    latents,
                    t,
                    prompt_embeds,
                    num_iter_steps=num_iter_steps,
        ...
        return InversionPipelineOutput(zT=latents, ori_image=image)
