...
class SDXLInversionPipeline_save_intermediate(StableDiffusionXLPipeline):
    @torch.no_grad()
    def inverse(
        self,
        ...
        with EmojiProgressBar(total=num_inference_steps, desc="DDIM Inversion") as pbar:
            for i in range(num_inference_steps):
                ...
                with torch.no_grad():
                    ...
                    # intermediate = InversionPipelineOutput(zT=latents/self.scheduler.init_noise_sigma, ori_image=image)
                    vae_recon = latents2image(self, latents)
                    vae_recon.save(os.path.join(path, "inv_{}.jpg".format(t)))
        # Offload all models
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                with torch.no_grad():
                    print("latents", latents.shape)
                    intermediate = latents2image(self, latents)
                    intermediate.save(os.path.join(path, "inf_{}.jpg".format(t)))
